...
%    AntennaSubsystem   : Turnstile, Mount
%    RFFrontEnd         : Coax (~1 m) -> LNA (mast-mounted) -> Coax (~4 m) -> Bias-T
...
%  Downlink: LRPT @ 137.1 / 137.9 MHz, QPSK, ~72 kbps
%
%  All system inputs are declared in groundstation_params.m. This script
%  sources that file and COMPUTES every derived quantity from it — no input
%  values are redefined here, so the parameters file is the single source
%  of truth.
% =========================================================================
...
% -------------------------------------------------------------------------
%  1. LOAD SHARED PARAMETERS  (single source of truth)
% -------------------------------------------------------------------------
run('groundstation_params.m');

% Derived constant
k_B_dB = 10*log10(k_B);            % Boltzmann constant (dBW/K/Hz) ~ -228.60
f_Hz   = f_MHz * 1e6;
lambda = c / f_Hz;

% -------------------------------------------------------------------------
%  2. SATELLITE TRANSMITTER
% -------------------------------------------------------------------------
...
el_deg_vec = [5, 10, 20, 30, 45, 90];   % Elevation angles to evaluate (deg)

% Slant range: d = -R*sin(el) + sqrt((R+h)^2 - R^2*cos^2(el))
R_km   = R_earth_km;
h_km   = alt_km;
...
% -------------------------------------------------------------------------
%  5. PROPAGATION LOSSES  (from params)
% -------------------------------------------------------------------------
...
fprintf('  Polarisation mismatch: %.1f dB  (turnstile CP vs sat RHCP)\n', L_pol_dB);
fprintf('  Misc / pointing      : %.1f dB\n', L_misc_dB);
fprintf('  Total additional     : %.1f dB\n\n', L_total_dB);

% -------------------------------------------------------------------------
%  6. GROUND STATION RECEIVE CHAIN  (computed from params)
% -------------------------------------------------------------------------
fprintf('--- GROUND STATION RECEIVE CHAIN (SysML: B-Raspi V1.0) ---\n');

% Cascaded noise figure (Friis). Only ~1 m of coax sits BEFORE the mast-mounted
% LNA; the ~4 m run to the SDR sits AFTER it and is suppressed by LNA gain:
%   F = L_pre + (F_lna-1)*L_pre + (F_sdr-1)*L_pre*L_post/G_lna
NF_lna_lin  = 10^(NF_lna_dB/10);
NF_sdr_lin  = 10^(NF_sdr_dB/10);
G_lna_lin   = 10^(G_lna_dB/10);
L_pre_lin   = 10^(L_coax_pre_dB/10);
L_post_lin  = 10^(L_coax_post_dB/10);

NF_cascade_lin = L_pre_lin + ...
                 (NF_lna_lin - 1) * L_pre_lin + ...
                 (NF_sdr_lin - 1) * L_pre_lin * L_post_lin / G_lna_lin;
...
fprintf('  [AntennaSubsystem]   Turnstile Gain : %.2f dBi\n', G_rx_dBi);
fprintf('  [RFFrontEnd]         Coax pre-LNA   : %.1f dB (1 m, in NF cascade)\n', L_coax_pre_dB);
fprintf('  [RFFrontEnd]         Coax post-LNA  : %.1f dB (4 m, suppressed by LNA gain)\n', L_coax_post_dB);
...
% -------------------------------------------------------------------------
%  7. LINK BUDGET vs ELEVATION
% -------------------------------------------------------------------------
fprintf('--- LINK BUDGET vs ELEVATION ANGLE ---\n');

% Required C/N0 = Eb/N0 + impl loss + 10log10(Rb)
...
for i = 1:length(el_deg_vec)
    ...
    Pr_dBW   = EIRP_dBW - fspl - L_total_dB + G_rx_dBi - L_coax_pre_dB;
...
fprintf('  Rx Antenna Gain (Turnstile)     : %.2f dBi\n', G_rx_dBi);
fprintf('  Cascaded System NF              : %.2f dB\n', NF_cascade_dB);
fprintf('  System G/T                      : %.2f dB/K\n', G_T_dB);
fprintf('  Required C/N0                   : %.2f dBHz\n', C_N0_req_dBHz);

% Worst case (min elevation = first entry, 5 deg)
Pr_worst_dBW = EIRP_dBW - FSPL_dB(1) - L_total_dB + G_rx_dBi - L_coax_pre_dB;
CN0_worst    = Pr_worst_dBW - k_B_dB - T_sys_dBK;
margin_worst = CN0_worst - C_N0_req_dBHz;

fprintf('\n  --- Worst Case (%d deg elevation) ---\n', el_deg_vec(1));
...
% -------------------------------------------------------------------------
%  10. PLOTS — Link Margin and Rx Power vs Elevation
% -------------------------------------------------------------------------
el_plot     = 2:1:90;
...
Pr_plot     = EIRP_dBW - FSPL_plot - L_total_dB + G_rx_dBi - L_coax_pre_dB;
...
%  System Architecture (from SysML model B-Raspi_v1_0.gaphor):
%    AntennaSubsystem   : Turnstile, Mount
%    RFFrontEnd         : Coax -> LNA (indoors at SDR) -> Bias-T
%    ReceiverSubsystem  : RTL-SDR V3, USB Interface
%    ProcessingSubsystem: Raspberry Pi 4, SatNOGS Client Lite
%
%  Link: Meteor M2-3 / M2-4 -> Ground Station (Dublin, IE)
...
% Cascaded noise figure (Friis). Coax sits BEFORE the LNA (LNA indoors at SDR),
% so the passive coax loss contributes its own loss as NF and scales the later
% stages:  F = L + (F_lna-1)*L + (F_sdr-1)*L/G_lna
NF_lna_lin = 10^(NF_lna_dB/10);
NF_sdr_lin = 10^(NF_sdr_dB/10);
G_lna_lin  = 10^(G_lna_dB/10);
...
% Worst case (min elevation = first entry, 5 deg)
Pr_worst_dBW = EIRP_dBW - FSPL_dB(1) - L_total_dB + G_rx_dBi - L_coax_dB;
CN0_worst    = Pr_worst_dBW - k_B_dB - T_sys_dBK;
margin_worst = CN0_worst - C_N0_req_dBHz;
