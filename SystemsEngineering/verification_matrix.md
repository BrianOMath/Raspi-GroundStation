...
| REQ-F-02 | Detect IQ completion; initiate processing without operator action | D | `process_meteor.sh` (inotifywait close_write trigger) + journalctl log of unattended pass processing | **PASS** | Verified continuously in operation since Jun 2026|
...
| REQ-P-01 | ≥ 80% of passes with max el ≥ 20° yield decoded imagery | T | Observation tally, station 4621, 12–24 July 2026: **48 decoded / 61 observations = 78.7%** | **FAIL (marginal)** | Shortfall of 1.3 percentage points. 95% CI ≈ 68–89%, i.e. the measured rate is not statistically distinguishable from the 80% requirement at this sample size. See REQ-P-01b and nonconformance 4 |
